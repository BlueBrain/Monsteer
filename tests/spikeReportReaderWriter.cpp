...
{
    BOOST_CHECK_THROW( monsteer::SpikeReportReader report0( brion::URI( "./bla" )),
                       std::runtime_error );

    boost::filesystem::path path( BBP_TESTDATA );
    path /= "local/README";
    BOOST_CHECK_THROW( monsteer::SpikeReportReader( brion::URI( path.string( ))),
                       std::runtime_error );
}
...
BOOST_AUTO_TEST_CASE( test_simple_load_static )
{
    ...
    monsteer::SpikeReportReader reader( brion::URI( path.string( )));
}

BOOST_AUTO_TEST_CASE( test_default_constructor )
{
    monsteer::Spikes spikes;
...
BOOST_AUTO_TEST_CASE( test_simple_read )
{
    ...
    monsteer::SpikeReportReader reader( brion::URI( path.string( )));
    const monsteer::Spikes& spikes = reader.getSpikes();
...
BOOST_AUTO_TEST_CASE( test_closed_window )
{
    ...
    const monsteer::Spikes& spikes = reader.getSpikes( 2.5f, 2.5f );
    BOOST_CHECK( spikes.empty() );
    BOOST_CHECK_EQUAL( spikes.size(), 0 );
    BOOST_CHECK( spikes.begin() == spikes.end( ));
}
...
BOOST_AUTO_TEST_CASE( test_out_of_window )
{
    ...
    const monsteer::Spikes& window = reader.getSpikes(start, start + 1 );
...
BOOST_AUTO_TEST_CASE( test_moving_window )
{
    ...
    while( !reader.hasEnded( ))
    {
        const monsteer::Spikes& spikes = reader.getSpikes(start, start + 1);
        if( !spikes.empty( ))
        {
...
BOOST_AUTO_TEST_CASE( TestSpikes_nest_spikes_read_write )
{
    ...
    monsteer::SpikeReportReader reader( brion::URI( path.string( )));
    const monsteer::Spikes& readSpikes = reader.getSpikes();

    TmpFile file( ".gdf" );

    monsteer::SpikeReportWriter writer( brion::URI( file.name ));
    writer.writeSpikes( readSpikes );
    writer.close();

    monsteer::SpikeReportReader reReader( brion::URI( file.name ));
    const monsteer::Spikes& reReadSpikes = reReader.getSpikes();
...
BOOST_AUTO_TEST_CASE( const_iterator_assignment_operator )
{
    ...
    monsteer::Spikes spikes = reader.getSpikes();

    monsteer::Spikes::const_iterator it;
    BOOST_CHECK( it != spikes.begin() );

    it = spikes.begin();
    monsteer::Spikes::const_iterator it2 = it;
...
BOOST_AUTO_TEST_CASE( const_iterator_copy_constructor )
{
    ...
    monsteer::Spikes::const_iterator it = spikes.begin();
    monsteer::Spikes::const_iterator it2( it );
